...
def main(args):
	...
	if not os.path.exists(dest_dir):
		os.mkdir(dest_dir)
	for person in files:
		person_image_path = os.path.join(root_dir,person)
		person_image_path_out = os.path.join(dest_dir,person) 
		...
		for person_img in os.listdir(person_image_path):
			...
			out_name = person_img.split(".")[0] + "_cropped_" + person_img.split(".")[-1]
			cv2.imwrite(os.path.join(person_image_path_out,out_name),cropped_face)
